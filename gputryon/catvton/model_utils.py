# Vendored, unmodified, from CatVTON (https://github.com/Zheng-Chong/CatVTON),
# by Zheng Chong et al., licensed CC BY-NC-SA 4.0. See ../README.md for
# attribution and license details -- this file is not original work. Trimmed
# to only what pipeline.py actually calls (init_adapter, get_trainable_module) --
# the original model/utils.py also has training-time-only helpers we don't need.
import torch
...
def init_adapter(unet,
                 ...
                 cross_attn_dim=None,
                 **kwargs):
    """Swaps every cross-attention processor in `unet` for `cross_attn_cls`
    (SkipAttnProcessor, disabling text conditioning entirely -- see
    attn_processor.py) while leaving self-attention processors as the
    standard scaled-dot-product implementation."""
...
def get_trainable_module(unet, trainable_module_name):
    """Returns the subset of `unet`'s modules that CatVTON's small adapter
    checkpoint (~50M params) actually trains/overrides -- here always
    "attention" (the self-attention blocks), which is what the published
    checkpoint's weights correspond to."""
